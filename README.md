...
| — | [`lb_reset_start.py`](archive/lb_reset_start.py) + [`lb_reset_probe.py`](archive/lb_reset_probe.py) | Fastest Reset probe: a reset starts on a **1×1** field with 1 drone and no items. Every language unlock is on (loops, functions, imports, lists, dicts, senses, debug, timing, costs, auto-unlock) and every farm unlock is at 0. Goal: Leaderboard = 2M Bone + 1M Gold |
| [`sim_full.py`](archive/sim_full.py) | [`full_run.py`](archive/full_run.py) | **Full Automation** (Fastest Reset leaderboard). `full_run` loops: buy wanted affordable upgrades in a staged priority order (Mazes ≤3 → Cactus → Megafarm (drones ≤ field width) → Dinosaurs → Expand → Mazes ≤6 → crops/tools (capped) → Leaderboard); farm the first missing item of the first producible target, or its inputs first. A simple sweep covers early crops. The production `farm_pumpkin`, `farm_cactus` (also the Weird Substance source), `farm_maze` and `farm_dinosaurs` take over later. 13 sim rounds fixed stalls (bushes never planted, field wiped between steps, Watering/Megafarm/Mazes grinds, Weird Substance starving the mazes). Round 13: **91,502 s** in sim. Real run: **Leaderboard in 91,739 s**, and it unlocked |
...
| Full Automation: `full_run` from a 1×1 reset (`sim_full`) | seed 1, reset unlocks, no items | — (did not finish in rounds 1–12) | **91,502 s** to Leaderboard. Real Fastest Reset run: 91,739 s. The 22×22 → 32×32 Pumpkin grind is ~56,000 s of it |
